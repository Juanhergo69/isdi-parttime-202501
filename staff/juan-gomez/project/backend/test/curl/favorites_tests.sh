#!/bin/bash

# ==============================================
# CONFIGURATION
# ==============================================
BASE_URL="http://localhost:3001/api"
ORIGIN="http://localhost:3000"

# User registration to get token
TIMESTAMP=$(date +%s)
TEST_USER="testuser_$TIMESTAMP"
TEST_EMAIL="test_$TIMESTAMP@example.com"
TEST_PASS="ValidPass123!"

echo -e "\n🔧 Registering test user..."
REGISTER_RESPONSE=$(curl -s -v -X POST "$BASE_URL/register" \
  -H "Content-Type: application/json" \
  -H "Origin: $ORIGIN" \
  -d '{
    "username": "'"$TEST_USER"'",
    "email": "'"$TEST_EMAIL"'",
    "password": "'"$TEST_PASS"'",
    "confirmPassword": "'"$TEST_PASS"'"
  }' 2>&1)

TOKEN=$(echo "$REGISTER_RESPONSE" | grep -o '"token":"[^"]*' | cut -d'"' -f4)
TEST_USER_ID=$(echo "$REGISTER_RESPONSE" | grep -o '"id":"[^"]*' | cut -d'"' -f4)

# Test game ID (we'll use Snake)
GAME_ID=1

# ==============================================
# UTILITY FUNCTIONS
# ==============================================

make_request() {
    local method=$1
    local endpoint=$2
    local data=$3
    local token=$4
    
    local cmd="curl -s -v -X $method '$BASE_URL$endpoint' \
        -H 'Content-Type: application/json' \
        -H 'Origin: $ORIGIN'"
    
    if [ -n "$token" ]; then
        cmd+=" -H 'Authorization: Bearer $token'"
    fi
    
    if [ -n "$data" ]; then
        cmd+=" -d '$data'"
    fi
    
    eval "$cmd" 2>&1
}

run_test() {
    local test_name=$1
    local response=$2
    local expected_status=$3
    
    echo -e "\n🔹 $test_name"
    echo "Response:"
    echo "$response" | grep -E 'HTTP|<|{'
    
    local status_code=$(echo "$response" | grep -oP 'HTTP/\d\.\d \K\d{3}')
    
    if [[ "$status_code" == "$expected_status" ]]; then
        echo "✅ PASSED (Expected $expected_status, Got $status_code)"
    else
        echo "❌ FAILED (Expected $expected_status, Got $status_code)"
    fi
}

# ==============================================
# FAVORITES TESTS
# ==============================================
echo -e "\n=============================================="
echo "⭐ FAVORITES TESTS"
echo -e "==============================================\n"

# 1. Get user favorites (should be empty initially)
GET_FAVORITES_RESPONSE=$(make_request "GET" "/users/$TEST_USER_ID/favorites" "" "$TOKEN")
run_test "1. FAVORITES - Get initial list (empty)" "$GET_FAVORITES_RESPONSE" 200

# 2. Add game to favorites
ADD_FAVORITE_RESPONSE=$(make_request "POST" "/users/favorites/add" '{
    "userId": "'"$TEST_USER_ID"'",
    "gameId": "'"$GAME_ID"'"
}' "$TOKEN")
run_test "2. FAVORITES - Add game (ID: $GAME_ID)" "$ADD_FAVORITE_RESPONSE" 200

# 3. Verify favorites after adding
GET_UPDATED_FAVORITES_RESPONSE=$(make_request "GET" "/users/$TEST_USER_ID/favorites" "" "$TOKEN")
run_test "3. FAVORITES - Verify game was added" "$GET_UPDATED_FAVORITES_RESPONSE" 200

# 4. Remove game from favorites
REMOVE_FAVORITE_RESPONSE=$(make_request "POST" "/users/favorites/remove" '{
    "userId": "'"$TEST_USER_ID"'",
    "gameId": "'"$GAME_ID"'"
}' "$TOKEN")
run_test "4. FAVORITES - Remove game (ID: $GAME_ID)" "$REMOVE_FAVORITE_RESPONSE" 200

# 5. Verify favorites after removal
GET_FINAL_FAVORITES_RESPONSE=$(make_request "GET" "/users/$TEST_USER_ID/favorites" "" "$TOKEN")
run_test "5. FAVORITES - Verify game was removed" "$GET_FINAL_FAVORITES_RESPONSE" 200

# ==============================================
# CLEANUP
# ==============================================
echo -e "\n🧹 Cleaning up test user..."
make_request "DELETE" "/users/$TEST_USER_ID" "" "$TOKEN" > /dev/null

# ==============================================
# FINAL SUMMARY
# ==============================================
echo -e "\n=============================================="
echo "🏁 TESTS COMPLETED"
echo -e "==============================================\n"

echo "📊 Summary:"
echo "- Test user: $TEST_EMAIL"
echo "- User ID: $TEST_USER_ID"
echo "- Generated token: ${TOKEN:0:30}..."
echo "- Tested game: Snake (ID: $GAME_ID)"
echo -e "\n✅ All tests completed successfully"