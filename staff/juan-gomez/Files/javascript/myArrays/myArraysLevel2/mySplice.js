...
function mySplice(array, start, end, ...splice) { //La función realizaría un splice de forma manual, generando un índice que eliminaría los parámetros ubicados entre entre star y end, y con posibilidad de añadir elementos a continuación de esas posiciones (splice). Ponemos tres puntos sobre splice para dejar la puerta abierta a la inclusión de varios elementos//
    ...
    var deletedElements = [] //Declaramos variable deletedElements, que contendrá el array de los elementos que van a ser eliminados del array//
    ...
    if (end !== splice.length) { //El if nos indica, que si la posición end no es igual a la longitud de los elementos que se quieren añadir al array//
        var counterPositions = splice.length - end //Declaramos variable counterPositions (contador de posiciones), que será la resta de la longitud de los elementos que se quieren añadir al array menos la posición end//
        if (counterPositions > 0) { //El if nos indica que si el contador de posiciones es superior a 0 (es decir, hay más elementos para agregar que eliminar)//
            for (var i = array.length - 1; i >= start + end; i--) { //Iteramos con un for a la inversa para desplazar hacia la derecha todos los elementos restantes del array, indicando el índice es la longitud del array - 1, y que este este es mayor o igual a la suma de las posiciones star y end//
            ...
            for (var i = start + end; i < array.length; i++) { //Iteramos con un for para desplazar hacia la izquierda todos los elementos restantes del array, indicando que el índice es la suma de las posiciónes star y end, y que estas son inferiores a la longitud del array//
    ...
    for (var i = 0; i < splice.length; i++) { //El for itera todos los elementos considerados en la variable splice (elementos que se quieren añadir al array tras la eliminación de los anteriores). La intención es retener este recorrido para añadir los nuevos elementos al array//
...
var controlResult1 = testNames.splice(0, 2, 'Copito') //Declaramos primera variable de control, donde realizamos el splice de testeo sobre el array de testeo. Devolvería los elementos eliminados con el metodo splice (en este caso, Juan (posición 0) hasta Sonia (sin incluirla en la eliminación, posición 2)//
var controlResult2 = mySplice(names, 0, 2, 'Copito') //Declaramos la segunda variable de control, donde aplicaremos el splice original sobre el array original, a través de nuestra función manual. Devolvería los elementos eliminados con el metodo splice (en este caso, Juan (posición 0) hasta Sonia (sin incluirla en la eliminación, posición 2)//
//***************************************************************************************************************************************************************//
