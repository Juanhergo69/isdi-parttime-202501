...
let names = ['Juan', ['Francisco', 'Loli', ['Dante', 'Germán']], 'Diana', ['Gonzalo', 'Angel', ['María', 'David']], 'Sonia', ['Lidia, Tere', ['Amira', 'Telma']], 'Marta'] //Declaramos variable con array original//
let testNames = ['Juan', ['Francisco', 'Loli', ['Dante', 'Germán']], 'Diana', ['Gonzalo', 'Angel', ['María', 'David']], 'Sonia', ['Lidia, Tere', ['Amira', 'Telma']], 'Marta']  //Declaramos variable de test con los elementos del array original//
//**************************************************************************************************************************************************************//

//***************************************************************************************************************************************************************//
//DECLARAMOS UNA FUNCIÓN QUE REALIZARÍA, DE MANERA MANUAL, EL MÉTODO FLAT//
const myFlat = (array, depthIndex = 1) => {
    ...
    if (depthIndex <= 0) { //El if nos indica que si la profundidad es menor o igual a 0, devuelve una copia del array original//
        let arrayCopy = [] //Declaramos variable arrayCopy como array vacío, y se rellenará según la iteración del for que sigue a continuación//
    ...
    let actualArray = [] //Declaramos variable actualArray como array vacío, y se rellenará según la siguiente iteración//
    for (let i = 0; i < array.length; i++) { //El for itera todos los elementos del array//
        actualArray[i] = array[i] //Y rellenamos actualArray con el resultado de la iteración del array//
    }

    let isInfinity = depthIndex === Infinity //Declaramos variable isInfinity, y le asiganmos el valor de que el índice de profunidad es igual a Infinity (no como concepto, si no como la totalidad de los anidados, es decir, debe ser considerado como número de la totalidad de anidados)//
    let nestingArray = true //Declaramos variable nestingArray como true para poder entrar en el bucle while. Según comprobaciones siguientes, verificaremos si existen o no anidaciones en el array//
    ...
    while ((isInfinity && nestingArray) || (!isInfinity && depthIndex > 0)) {//El while se continuará ejectuando, mientras que isInfinity sea true y existan anidaciones, o también en el caso de que isInfinity sea false y que el índice de profundidad sea mayor a 0//
        ...
        let tempArray = [] //Declaramos variable tempArray como array vacío, y se rellenerá según las comprobaciones y condiciones siguientes//
        for (let i = 0; i < actualArray.length; i++) { //El for itera un índice basado en la longitud de actualArray//
            if (actualArray[i] !== null && actualArray[i].constructor === Array) { //El if nos indica, que si el indice recorrido de actualArray es distinto a null (también valdría undefinded), y que además, es reconocido por el comando .constructor como un Array//
                for (let j = 0; j < actualArray[i].length; j++) {//Iteramos con con un for un nuevo índice basado en la longitud del índice de actualArray//
...
const controlResult1 = testNames.flat(1) //Declaramos primera variable de control, donde realizamos el flat de testeo sobre el array de testeo. Devolvería el array desanidado hasta la profundidad indicada (en este caso, solo desanidaría el primer anidado)//
const controlResult2 = myFlat(names, 1) //Declaramos la segunda variable de control, donde aplicaremos el flat original sobre el array original, a través de nuestra función manual (no desarrollada aún)//
//***************************************************************************************************************************************************************//
